...
/** Insert Cloudinary resize transforms into a URL */
function cloudinaryResize(url: string, width: number, aspectRatio: string): string {
  return url.replace(
    "/image/upload/",
    `/image/upload/w_${width},c_fill,g_center,ar_${aspectRatio}/`
  );
}
...
export default function DiscoverRestaurantCard({
  ...
  return (
    <div
      ...
      onClick={onClick}
    >
      {/* Image — compact strip when collapsed, full 3:2 when selected */}
      {restaurant.imageUrl && (
        <div className={`w-full overflow-hidden transition-all duration-200 ${isSelected ? "aspect-[3/2]" : "h-20"}`}>
          <img
            src={cloudinaryResize(restaurant.imageUrl, 600, "3:2")}
            alt={venue.name || restaurant.title}
